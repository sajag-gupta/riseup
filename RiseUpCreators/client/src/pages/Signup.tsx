            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    type="button"
                    onClick={() => form.setValue("userType", "fan")}
                    className={`p-4 rounded-lg border-2 transition-all ${
                      form.watch("userType") === "fan"
                        ? "border-emerald-500 bg-emerald-500/10"
                        : "border-gray-600 bg-gray-800 hover:border-gray-500"
                    }`}
                  >
                    <Headphones className="w-8 h-8 mx-auto mb-2 text-emerald-400" />
                    <p className="text-white font-medium">Music Fan</p>
                    <p className="text-gray-400 text-xs">Listen & discover</p>
                  </button>
                  <button
                    type="button"
                    onClick={() => form.setValue("userType", "creator")}
                    className={`p-4 rounded-lg border-2 transition-all ${
                      form.watch("userType") === "creator"
                        ? "border-emerald-500 bg-emerald-500/10"
                        : "border-gray-600 bg-gray-800 hover:border-gray-500"
                    }`}
                  >
                    <Music className="w-8 h-8 mx-auto mb-2 text-emerald-400" />
                    <p className="text-white font-medium">Music Creator</p>
                    <p className="text-gray-400 text-xs">Create & share</p>
                  </button>
                </div>
