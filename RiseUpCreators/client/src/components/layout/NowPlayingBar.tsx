...
export function NowPlayingBar() {
  const { currentTrack, isPlaying, togglePlayPause, volume, setVolume, currentTime, duration, seek, stopTrack, skipNext, skipPrevious, queue, currentIndex } = usePlayer();
  ...
  const handleSkipNext = () => {
    if (queue.length > 0 && currentIndex < queue.length - 1) {
      ...
      toast({ title: "No next track in queue" });
    }
  };
  ...
  const handleSkipPrevious = () => {
    if (queue.length > 0 && currentIndex > 0) {
      ...
      toast({ title: "No previous track in queue" });
    }
  };
  ...
  const handleLike = async () => {
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to like track');
      }
    } catch (error) {
      console.error('Like error:', error);
      toast({ 
        title: "Failed to like track", 
